create or replace procedure Display_Hall(n in number)
	...
	if(n = 1) then
		...
		for cursor in (select * from Hall@Banani) loop
			dbms_output.put_line(cursor.HallID || ' ' || cursor.HallName || ' ' || cursor.category || ' ' || cursor.totalSeats);
		end loop;
	else 
...
create or replace procedure Display_Show(n in number)
	...
	if(n = 1) then
		...
		for cursor in (select * from Show@Banani) loop
		dbms_output.put_line(cursor.ShowId || ' ' || cursor.ShowTime);
		end loop;
...
create or replace procedure Display_Schedule(n in number)
	...
	if(n = 1) then
		...
		for cursor in (select * from Schedule@Banani) loop
		dbms_output.put_line(cursor.V_ID || ' ' || cursor.H_ID || ' ' || cursor.S_ID || ' ' || cursor.M_ID || ' ' || cursor.ShowDate);
		end loop;
	else 
		dbms_output.put_line('Invalid Input.');
...
create or replace procedure Display_Seat(n in number)
	...
	if(n = 1) then
		...
		for cursor in (select * from Seat@Banani) loop
		dbms_output.put_line(cursor.SeatID || ' ' || cursor.V_ID || ' ' || cursor.Position || ' ' || cursor.Price);
		end loop;
	else 
		dbms_output.put_line('Invalid Input.');
...
create or replace procedure Display_Movies(n in number)
	...
	if(n = 1) then
		...
		for cursor in (select * from Movies@Banani) loop
		dbms_output.put_line(cursor.MovieID || ' ' || cursor.Title || ' ' || cursor.Category || ' ' || cursor.ReleaseDate || ' ' || cursor.Language);
		end loop;
...
create or replace procedure Buy_Tickets(n in number)
	...
	if(n = 1) then
		...
		select max(ticketID) into TestP from Ticket @Panthapath;
        Insert into Ticket@Panthapath values(TestP + 1,BuyTickets_VID,BuyTickets_HID,BuyTickets_SID,BuyTickets_STID,BuyTickets_MID,BuyTickets_Date);
        commit;
	elsif(n = 2) then
		...
		select max(ticketID) into TestBanani from Ticket @Banani;
        Insert into Ticket@Banani values(TestBanani + 1,BuyTickets_VID,BuyTickets_HID,BuyTickets_SID,BuyTickets_STID,BuyTickets_MID,BuyTickets_Date);
        commit;
...
create or replace procedure Display_Manager(n in number)
	...
	if(n = 1) then
		...
		for cursor in (select * from Manager@Banani) loop
		dbms_output.put_line(cursor.ManagerID || ' ' || cursor.ManagerName || ' ' || cursor.ManagerContact);
		end loop;
...
create or replace procedure Movie_Specific_Information(n in number)
	...
	begin
		...
		if(n = 1) then
			...
			for cursor in (select * from TestViewBanani) loop
				select showTime into show from show@Banani where showId = (cursor.s_id);
				select hallName into hall from hall@Banani where hallId = (cursor.h_id);
				select totalSeats into totSeats from hall@Banani where hallId = (cursor.h_id);
				select count(ticketID) into soldTickets from Ticket@Banani where (v_id = n and s_id = cursor.s_id and m_date = cursor.showDate and h_id = cursor.h_id);
				availableTickets := totSeats - soldTickets;
				dbms_output.put_line(show || '     ' || hall || '      ' || cursor.showDate || '    '||  availableTickets);
			end loop;
		end if;
